...
export default function PostCard({ item }) {
  ...
  React.useEffect(()=>{
       console.log("PostCardForLike Checking",item);
  },[])
     ...
     {item.image && <CardMedia
      ...
      <CardActions className="flex justify-between" disableSpacing>
        <div>
          ...
          {item?.likedByRequser ? (
            <div className="text-sm flex gap-1 ">  {item.liked.length} likes<FavoriteIcon  sx={{ color: pink[500] }} /> </div>
          ) : (
            <div className="text-sm flex gap-1">  {item.liked.length} likes <FavoriteBorderIcon /> </div>
          )}
        </IconButton>
